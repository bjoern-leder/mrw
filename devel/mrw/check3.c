...
int main(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      printf("\n");
      printf("Consitency of mrw1 and mrw2\n");
      printf("------------------------------\n\n");
   ...
   for (irw=0;irw<2;irw++)
   {
      ...
      for (isp=0;isp<3;isp++)
      {
         ...
         if (irw==0)
         {
            ...
            ms.d1=dm;
            ms.mu1=mu0;
            z0=mrw1(ms,0,isp,sqnp0,&sqne0,status);

            start_ranlux(0,8911+isp);
            ms.m1=m0/2.0;
            ms.d1=dm/2.0;
            ms.mu1=mu0/2.0;
            z1=mrw1(ms,0,isp,sqnp0+1,&sqne0,status);
            ...
            ms.mu2=mu0/2.0;
            start_ranlux(0,8911+isp);
            mrw2(ms,0,ispp,lnw1,sqnp1,&sqne1,status);
         }
         else if (irw==1)
         {
            ms.m1=m0;
            ms.d1=dm;
            ms.mu1=mu0;
            z0=mrw1(ms,1,isp,sqnp0,&sqne0,status);

            start_ranlux(0,8911+isp);
            ms.m1=m0/2.0;
            ms.d1=dm/2.0;
            ms.mu1=mu0/2.0;
            z1=mrw1(ms,1,isp,sqnp0+1,&sqne0,status);
            ...
            ms.mu2=mu0/2.0;
            start_ranlux(0,8911+isp);
            mrw2(ms,1,ispp,lnw1,sqnp1,&sqne1,status);
         }
