...
static void alloc_vbufs(void)
{
   ...
   for (ifc=0;ifc<8;ifc++)
   {
      ...
      MPI_Send_init((float*)(snd_buf_int[ifc]),2*Ns*nbbo[ifc],
                    MPI_FLOAT,saddr,tag,MPI_COMM_WORLD,&snd_req_int[ifc]);
      MPI_Recv_init((float*)(rcv_buf_int[ifc]),2*Ns*nbbe[ifc^0x1],
   ...
   for (ifc=0;ifc<8;ifc++)
   {
      ...
      MPI_Send_init((float*)(snd_buf_ext[ifc]),2*Ns*nbbe[ifc],
                    MPI_FLOAT,saddr,tag,MPI_COMM_WORLD,&snd_req_ext[ifc]);
      MPI_Recv_init((float*)(rcv_buf_ext[ifc]),2*Ns*nbbo[ifc^0x1],
